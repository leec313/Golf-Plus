...
class BlogTests(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(title='Test Post', content='Test Content', author=self.user, slug='royal-portrush-golf-club')
    ...
    def test_post_detail_view(self):
        """Simulating a GET request to the 'post_detail' URL for the created post"""
    ...
    def test_newsletter_subscription(self):
        """Simulating a POST request to the 'subscribe_newsletter' URL with form data"""
    ...
    def test_post_update_view(self):
        ...
        response = self.client.post(self.update_url, {'title': updated_title, 'content': 'Updated Content'})
        self.assertEqual(response.status_code, 302)
    ...
    def test_post_comment(self):
        ...
        response = self.client.post(reverse('post_detail', kwargs={'slug': self.post.slug}),
                                    {'body': 'Test Comment'})

        # Asserting that the response status code is 200 (OK) after posting a comment
        self.assertEqual(response.status_code, 200)

        # Check if the comment is added to the database
        self.assertTrue(Comment.objects.filter(body='Test Comment', post=self.post, user=self.user).exists())
    ...
    def test_comment_update_view(self):
        ...
        response = self.client.get(reverse('update_comment', kwargs={'pk': self.comment.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'comment_update.html')

        # Simulating a POST request to the 'comment_update' URL with updated comment data
        updated_body = 'Updated Test Comment'
        response = self.client.post(reverse('update_comment', kwargs={'pk': self.comment.pk}),
                                    {'body': updated_body})
        self.assertEqual(response.status_code, 302)
    ...
    def test_post_like_view(self):
        ...
        self.post.refresh_from_db()
        self.assertEqual(self.post.likes.filter(id=self.user.id).exists(), True)
